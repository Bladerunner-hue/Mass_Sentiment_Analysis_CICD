...
class TestBatchService:
    ...
    def test_process_dataframe_adds_sentiment_columns(self):
        ...
        df = pd.DataFrame({
            'Text': ['I love this!', 'I hate this.', 'It is okay.']
        })

        result_df = self.service.process_dataframe(
            df, 'Text', include_emotions=False
        )
    ...
    def test_process_dataframe_adds_emotion_columns(self):
        ...
        df = pd.DataFrame({
            'Text': ['I am so happy!', 'I am very sad.']
        })

        result_df = self.service.process_dataframe(
            df, 'Text', include_emotions=True
        )
    ...
    def test_process_dataframe_progress_callback(self):
        ...
        df = pd.DataFrame({
            'Text': ['Text ' + str(i) for i in range(10)]
        })
    ...
    def test_process_file_returns_stats(self, tmp_path):
        ...
        result = self.service.process_file(
            str(input_path), include_emotions=False
        )
