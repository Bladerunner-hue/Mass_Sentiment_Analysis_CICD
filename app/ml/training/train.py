...
class TrainingRunTracker:
    ...
    def __init__(self, db_url: Optional[str] = None):
        self.db_url = db_url or os.getenv("TRAINING_DB_URL")
        self._conn = None
    ...
    def start_run(self, name: str, params: Dict) -> Optional[int]:
        ...
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO training_runs (run_name, params, status) VALUES (%s, %s, %s) RETURNING id",
            (name, params, "running"),
        )
    ...
    def log_epoch(self, run_id: Optional[int], epoch: int, metrics: Dict) -> None:
        if not run_id or not self._connect():
            return
        cur = self._conn.cursor()
        cur.execute(
            """
            INSERT INTO epoch_metrics (training_run_id, epoch, train_loss, val_loss, sentiment_acc, emotion_acc)
            VALUES (%s, %s, %s, %s, %s, %s)
            """,
            (
                run_id,
    ...
    def finish_run(self, run_id: Optional[int], status: str, best: Dict) -> None:
        if not run_id or not self._connect():
            return
        cur = self._conn.cursor()
        cur.execute(
            ...
            SET status=%s, best_val_loss=%s, best_sentiment_acc=%s, best_emotion_acc=%s, finished_at=NOW()
            WHERE id=%s
            """,
            (
                status,
...
def _prepare_dataloaders(
    ...
    if config.val_path:
        val_df = pd.read_csv(config.val_path).dropna(subset=["text", "sentiment", "emotion"])
    else:
...
def train_model(config: TrainerConfig) -> str:
    ...
    tokenizer.build_vocab(pd.read_csv(config.train_path)["text"].dropna().tolist())
    ...
    run_id = tracker.start_run("custom-bilstm-attention", vars(config))
    ...
    tracker.finish_run(run_id, "completed", best)
