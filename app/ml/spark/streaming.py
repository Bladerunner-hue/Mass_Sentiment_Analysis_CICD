...
class StreamConfig:
    ...
    tokenizer_path: str = field(
        default_factory=lambda: os.getenv("CUSTOM_TOKENIZER_PATH", "models/tokenizer.pkl")
    )
...
class SparkStreamingService:
    ...
    def start_twitter_stream(
        self, bearer_token: str, keywords: List[str], output_mode: str = "append"
    ) -> str:
        ...
        stream_df = (
            spark.readStream.format("rate")  # Demo source - replace with custom Twitter source
        ...
        stream_df = (
            ...
            .withColumn(
                ...
                F.concat(
                    ...
                    F.element_at(
                        F.array(*[F.lit(k) for k in keywords]),
            ...
            .withColumn("retweet_count", (F.rand() * 100).cast("long"))
            .withColumn("like_count", (F.rand() * 500).cast("long"))
        )

        # Apply sentiment analysis
        analyzed_df = stream_df.withColumn("analysis", sentiment_udf(F.col("text"))).select(
            F.col("tweet_id"),
            F.col("text"),
            F.col("author_id"),
            F.col("created_at"),
        ...
        query = (
            analyzed_df.writeStream.outputMode(output_mode)
    ...
    def start_reddit_stream(
        ...
        stream_df = (
            ...
            .withColumn("created_at", F.current_timestamp())
            .withColumn("score", (F.rand() * 1000).cast("long"))
        )
    ...
    def start_news_stream(self, rss_feeds: List[str], output_mode: str = "append") -> str:
    ...
    def _write_to_postgres_and_parquet(self, batch_df, batch_id: int):
        ...
        try:
            ...
            partitioned_df = batch_df.withColumn("date", F.to_date(F.col("created_at"))).withColumn(
                "hour", F.hour(F.col("created_at"))
            )

            (
                partitioned_df.write.partitionBy("source", "date", "hour")
                .mode("append")
...
def start_all_streams(
    ...
    if twitter_keywords:
        ...
        if bearer_token:
            stream_ids["twitter"] = streaming_service.start_twitter_stream(
                bearer_token=bearer_token, keywords=twitter_keywords
            )
    ...
    if rss_feeds:
        stream_ids["news"] = streaming_service.start_news_stream(rss_feeds=rss_feeds)
