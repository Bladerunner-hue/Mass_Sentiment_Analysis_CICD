...
class XTwitterService:
    ...
    def get_trending_topics(self, woeid: int = 1) -> Dict[str, Any]:  # 1 = Worldwide
    ...
    def stream_sample(self, callback, duration_seconds: int = 60) -> Dict[str, Any]:
        ...
        params = {"tweet.fields": "created_at,author_id,lang,source"}
        ...
        try:
            with self._session.get(
                url, params=params, stream=True, timeout=duration_seconds + 10
            ) as response:
                response.raise_for_status()
    ...
    def collect_for_sentiment_analysis(
        self, query: str, target_count: int = 1000, languages: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        ...
        while len(all_tweets) < target_count:
            result = self.search_recent_tweets(query=query, max_results=100, next_token=next_token)
